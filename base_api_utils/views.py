...
from base_api_utils.serializers.v2.query_params import (
    parse_request_trees,
    should_expand,
    subtree,
)
...
class ReadWriteSerializerMixin:
    ...
    def get_serializer(self, *args, **kwargs):
        ...
        if has_data:
            serializer_class = (
                self._action_write_cls(action) or self._default_write_cls()
            )
...
class BaseView(ReadWriteSerializerMixin, ModelViewSet):
    ...
    def apply_ordering(self, queryset):
        ordering_param = getattr(settings, "REST_FRAMEWORK", {}).get(
        ...
        if ordering:
            ...
            ordering_list = (
                self.ordering if isinstance(self.ordering, list) else [self.ordering]
            )
        else:
            return queryset
            ...
            if isinstance(self.ordering_fields, dict):
                ...
                if mapped_field:
                    ordering_fields.append(
                        f"-{mapped_field}" if is_desc else mapped_field
                    )
